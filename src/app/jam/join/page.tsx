...
function JoinJamContent() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const codeParam = searchParams.get("code");

  const [code, setCode] = useState(codeParam || "");
  const [preview, setPreview] = useState<JamPreview | null>(null);
  ...
  async function handleJoin() {
    ...
    try {
      const res = await fetch(`/api/jams/${preview.id}/join`, {
        method: "POST",
  ...
  return (
    <main className="gradient-bg min-h-screen flex flex-col items-center justify-center px-5 py-6">
      <div className="w-full max-w-md">
        <h1 className="text-2xl font-bold text-text-primary text-center mb-6">Join a Deep Dig</h1>

        {isLoading && (
          <div className="text-center">
            <p className="text-sm text-text-secondary">Looking up invite code...</p>
          </div>
        )}
        ...
        {!isLoading && preview && (
          <div className="space-y-4">
            <div className="glass rounded-xl p-5 text-center">
              <h2 className="text-lg font-semibold text-text-primary mb-1">{preview.name}</h2>
              {preview.description && (
                <p className="text-sm text-text-secondary mb-3">{preview.description}</p>
              )}
              <p className="text-sm text-text-secondary mb-3">
                Created by {preview.owner.displayName} &middot;{" "}
