...
const dbPath = path.join(dataDir, "jamjar.db");

const sqlite = new Database(dbPath);
sqlite.pragma("journal_mode = WAL");

// Try to preserve grayson's real Spotify tokens before wiping
let savedTokens: { accessToken: string; refreshToken: string; tokenExpiresAt: number } | null = null;
try {
  const row = sqlite.prepare(
    "SELECT access_token, refresh_token, token_expires_at FROM users WHERE spotify_id = ?"
  ).get("grayson.adams") as { access_token: string; refresh_token: string; token_expires_at: number } | undefined;
...
sqlite
  .prepare(
    `INSERT INTO jams (id, name, description, image_url, spotify_playlist_id, owner_id, invite_code, archive_playlist_id, archive_threshold, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  )
  .run(
    jamId,
...
const insertMember = sqlite.prepare(`
  INSERT INTO jam_members (id, jam_id, user_id, joined_at) VALUES (?, ?, ?, ?)
`);

// Owner is a member
insertMember.run(nanoid(), jamId, ownerUserId, oneHourAgo);

// Grayson joined later
insertMember.run(nanoid(), jamId, myUserId, thirtyMinAgo);

// ─── Jam Tracks ─────────────────────────────────────────────────────────────

const insertTrack = sqlite.prepare(`
  INSERT INTO jam_tracks (id, jam_id, spotify_track_uri, spotify_track_id, track_name, artist_name, album_name, album_image_url, duration_ms, added_by_user_id, added_at, removed_at, archived_at)
  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
`);
...
insertTrack.run(
  nanoid(), jamId,
  "spotify:track:6guXhXMAHU4QYaEsobnS6v", "6guXhXMAHU4QYaEsobnS6v",
...
insertTrack.run(
  nanoid(), jamId,
  "spotify:track:4u7EnebtmKWzUH433cf5Qv", "4u7EnebtmKWzUH433cf5Qv",
...
insertTrack.run(
  nanoid(), jamId,
  "spotify:track:5CQ30WqJwcep0pYcV4AMNc", "5CQ30WqJwcep0pYcV4AMNc",
...
insertTrack.run(
  nanoid(), jamId,
  "spotify:track:40riOy7x9W7GXjyGp4pjAv", "40riOy7x9W7GXjyGp4pjAv",
...
insertTrack.run(
  nanoid(), jamId,
  "spotify:track:7HD1jkMlfB78DOCGmHbKR4", "7HD1jkMlfB78DOCGmHbKR4",
...
insertTrack.run(
  nanoid(), jamId,
  "spotify:track:0LN0ASTtcMNRYWfHMgsFSS", "0LN0ASTtcMNRYWfHMgsFSS",
...
const playlistId = "4ZnPYsKOqPV2qP93VJDzTU";
...
async function syncTracksToSpotify() {
  ...
  console.log(`\n  Syncing tracks to Spotify playlist ${playlistId}...`);
  const existingUris = new Set<string>();
  let url: string | null = `${SPOTIFY_API}/playlists/${playlistId}/tracks?limit=50&fields=items(track(uri)),next`;
